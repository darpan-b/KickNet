import os
import platform  # <--- NEW: To detect OS
import time      # <--- NEW: For safety delay
...
def shutdown_machine():
    """
    Detects the operating system and shuts down the machine 
    after a safety delay.
    """
    print("\n" + "="*40)
    print("BENCHMARKING COMPLETE.")
    print("System will shut down in 60 seconds.")
    print("Press CTRL+C immediately to cancel shutdown.")
    print("="*40 + "\n")
    
    try:
        # 60-second safety buffer to cancel if you are watching
        time.sleep(60) 
        ...
        if system_os == "Windows":
            ...
            # Note: You may need to run the script with 'sudo' for this to work on Linux
            os.system("shutdown now")
        else:
            print(f"Unknown OS '{system_os}'. Automatic shutdown skipped.")
...
if __name__ == "__main__":
    ...
    # ~ benchmark_ppo()
    # ~ try:   
        # ~ # Only shuts down if all benchmarks finish successfully
        # ~ shutdown_machine()
        
    # ~ except Exception as e:
        # ~ print(f"\nAn error occurred during execution: {e}")
        # ~ print("Automatic shutdown aborted.")
